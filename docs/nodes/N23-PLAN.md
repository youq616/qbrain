...
**Plan audit**: PASS (`N23-PLAN-AUDIT.md`, Claude Code, 2026-08-08; audited draft SHA-256 `9601d6bc7665b52ac5871073d900c6b61763d1c5c440d88df68c8fbd6397a940`; audit SHA-256 `8da7e7af958e4d776885970c4cfddc581376582e94c3797b9309a43a5297b714`)
**Outcome audit**: PASS (fresh Claude Code outcome audit 2026-08-15, dispatched in N30 outcome phase; see N23-HARD-AUDIT.md)
**Wave**: Wave 5 (N20-N23); N23 is independent of N20-N22 after the dependencies above are complete
**Process note**: This is a retrospective re-verification and correctness closure required by the current node loop. The short 2026-07-27 plan, the 2026-07-29 N29 reconciliation audits, existing implementation/tests, and historical ledger claims are background only. They do not approve this plan and cannot satisfy its outcome gate. The first approved revision is archived as `N23-PLAN-APPROVED-BASELINE.md` with its matching `N23-PLAN-AUDIT-BASELINE.md`; the historical outcome artifact is archived as `N23-HARD-AUDIT-BASELINE.md`; those files and `PRE-CORRECTIVE-SCHEMA-GATE.json` are immutable historical facts. No N23 production, test, verifier, evidence, or ledger change may begin while this revision is `draft` or while its fresh plan audit is missing or FAIL.
...
The immutable gate is a historical boundary, not a license to overwrite current shared inputs. The current gate check found two finite post-gate shared-input drifts that are outside the original N23 corrective set: `scripts/build-cl.ps1` (gate SHA-256 `553a1989952d346fce8f836146377aec6d0db29e143893c3ac7fd28869b15070`, 3605 bytes; observed pre-revalidation SHA-256 `68da7dac59f989de658fe09fd86e20bc19632c64af65ff6edab10de0fcedabc8`, 3611 bytes, last write `2026-08-06T13:46:44.5746590Z`) and `src/qbrain/ops/registry.cpp` (gate SHA-256 `e3fb8bad77a88677d6ae23a2eba626e98d47ab41c0659cbac1d724688b134a18`; observed current SHA-256 `f96f1b1ec44fb9ad8e5a1b0e5922d3592b226053da306c7f3ff6c738ec91d7ff`, 1559 bytes, last write `2026-08-04T19:53:02Z`). They are handled only by the append-only sidecar contract below. Any third shared-input drift, a second registry change, or any change to the immutable gate returns this plan to `draft` and requires another fresh plan audit; the verifier may not expand an allowlist in place.

## Post-gate shared-input revalidation

After this revision receives a fresh Claude Code plan-audit PASS and is changed to `approved`, the parent must preserve the two archives and the immutable gate, then perform the following finite sequence before editing `scripts/n23-verify.ps1` or creating pending evidence:

1. Remove the unsafe process-management block from `scripts/build-cl.ps1`. The exact allowed change is to stop querying `Win32_Process` and to stop/kill/restart any `qbrain.exe`; the build must instead let MSVC/link fail normally when a target executable is locked. No source list, compiler flags, output path, or other process-control behavior may change. The resulting file hash, byte count, mtime, and an exact bounded diff hash are recorded in the sidecar.
2. Do not edit `src/qbrain/ops/registry.cpp` in N23. Bind its observed SHA-256, byte count, creation time, attributes, and last-write time in the sidecar, and fail closed if any of those values change before or during the native run.
3. Create `docs/nodes/n23-evidence/POST-GATE-SHARED-INPUT-REVALIDATION-v1.json` exactly once, after the build-script safety edit and before any N23 verifier/evidence edit. It must use strict UTF-8 JSON with no duplicate or unknown fields, include the immutable gate hash and completion time, the archived approved-plan/audit hashes, the current approved-plan/audit hashes, both paths' before/after file facts, and the exact bounded delta hash. A pre-existing v1 file, a changed v1 file, a second sidecar, or a path containing an N30 component is a hard failure; the sidecar is evidence, not an audit verdict.
4. The revised verifier must split frozen historical governance from current governance, validate the sidecar and exact two-path set, re-hash every bound file at each stage boundary, and retain the original N23 corrective input set unchanged. It must record the sidecar hash in `PREBUILD-MANIFEST.json`, `EVIDENCE-MANIFEST.json`, and `VERIFY-REPORT.md`.
5. The sidecar is not a replacement pre-corrective gate. If `scripts/build-cl.ps1` or `src/qbrain/ops/registry.cpp` changes again, retain v1, return this plan to `draft`, revise the finite contract, and obtain a new Claude Code plan-audit PASS before any further verifier/evidence work.
...
6. `scripts/n23-verify.ps1` and `docs/nodes/n23-evidence/` with the immutable pre-corrective schema gate, the append-only `POST-GATE-SHARED-INPUT-REVALIDATION-v1.json` sidecar, a frozen scoped prebuild manifest, native production/test build logs, two full-suite logs, focused N23 markers, selected/decoy snapshot evidence, platform/compiler evidence, final manifest, and `VERIFY-REPORT.md`. The verifier validates frozen historical governance, current approved governance, the exact sidecar, the safe build-script delta, and all evidence before writing pending/final state; it never writes an audit verdict or node/ledger status.
...
8. Native regression evidence:
   ...
   - Before native evidence, validate the append-only sidecar: the build script contains no process enumeration or termination, the registry source matches its bound current facts, the immutable gate and archived governance remain byte-identical, and a second shared-input drift fails closed.

## Acceptance assertions (falsifiable)

1. The immutable pre-corrective gate proves that an isolated current binary returned `ok=true` and schema v12 before every corrective N23 edit, binds the approved plan/audit/binary/HEAD/scoped inputs, proves production-data isolation and cleanup, and remains byte-identical. The final verifier separately validates the archived historical governance, current approved governance, and append-only two-path sidecar before accepting the native evidence; N23 introduces no DDL/version migration.
...
13. Evidence manifests bind the approved plan, plan audit, immutable gate, sidecar, HEAD, scoped inputs, commands, platform, binaries, logs, counts, and hashes; the verifier has no audit/status/ledger authority and detects stale or mismatched evidence.
...
- A failing build, test, evidence check, or outcome audit leaves the plan `approved` (or returns it to `draft` if the plan or shared-input boundary is wrong), leaves ledger notes unreconciled, and blocks the next wave. The immutable gate, archived governance, and any captured sidecar are retained; they are never rewritten to make a later run pass.
...
- The native build helper must never enumerate, stop, kill, or restart a user-owned `qbrain.exe`; a locked output is a normal build failure. Sidecar and evidence files contain hashes and bounded metadata only, not secrets or provider/model configuration.
...
- `scripts/build-cl.ps1` and `src/qbrain/ops/registry.cpp` are additional serialized shared inputs for this revision. No agent may change either after sidecar capture; a further change invalidates the sidecar and returns this plan to `draft`.
