...
如果同句仍为 active，但原支持全部自然过期，新的独立有效事件可以给原事实附加支持，
保留 fact_id 并增加 revision；前提是全部历史支持仍通过原文、来源、角色、哈希和页面
完整性校验。旧到期时间不改变，读取仅返回现在有效的新证据。被撤回/替代、遭篡改、
页面删除或历史支持部分无效的记录不会因此恢复；16份物理支持上限也不会绕过。
这不是重放已过期事件：旧事件本身仍被拒绝，必须是新的当前有效证据。
