  ...
  try {
      ...
      if(view=="usage_batch") {
        for(const auto& [key,value]:c.args)
          if(key!="source_id" && key!="view" && key!="payload")
            throw memory::Error("fact_unexpected_argument");
        return output(memory::usage_batch(*c.brain,*source,batch_payload(get(c,"payload")),false));
      }
    ...
    if(action.rfind("fact_",0)==0) {
      if(c.args.count("event_id") || c.args.count("method") || c.args.count("manual"))
        throw memory::Error("fact_unexpected_argument");
      if(action=="fact_usage_batch")
        return output(memory::usage_batch(*c.brain,*source,batch_payload(get(c,"payload")),true));
