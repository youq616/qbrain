# N47R -> N47X native upgrade, synthetic facts and explicit use receipts only.
[CmdletBinding()]
param(
...
if((Hash $old) -cne 'e7158949d805a0a25157bfb720561e4b21e80a433a6c7f031c60ee3bbaa746c5'){throw 'Wrong original release.'}
# OldPackage is a verified ZIP; expand both packages into fresh Unicode paths.
$root=Join-Path ([IO.Path]::GetTempPath()) ('n47x-'+[Guid]::NewGuid().ToString('N'))
...
try{
 ...
 Check ($manifest.product_source -ceq 'b810d6898dcbbcd49bdbbed1d8473a3ac1c063e4' -and (Hash $exe) -ceq $manifest.binary_sha256) 'new package source and executable identity'
 Check ((Hash $exe) -cne (Hash $oldExe)) 'new package does not reuse the old c26 executable'
 ...
 Set-Location $before
 $record=[pscustomobject]@{schema='qbrain-n47x-upgrade-v1';result=$(if($ok){'PASS'}else{'FAIL'});shell_major=$PSVersionTable.PSVersion.Major;checks=@($checks);failure=$failure;old_zip_sha256=(Hash $old);new_zip_sha256=(Hash $new);binary_sha256=(Hash $exe);script_sha256=(Hash $PSCommandPath);real_client_verified=$false}
