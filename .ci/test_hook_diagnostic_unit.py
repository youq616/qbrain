"""Execute real Hook diagnostic inspection unit tests; no claim of independently measured model egress."""
import argparse
import hashlib
import json
import os
...
import sys
import tempfile
from test_hook_diagnostic_process import provenance
from validate_hook_diagnostic_report import validate_unit_payload
...
def main():
    ...
    script=Path(__file__).resolve();test=script.parents[1]/'tests/test_n47k.cpp'
    report={'result':'FAIL','native_windows':os.name=='nt','scope':'synthetic C++ Hook diagnostic inspection tests; no live host acceptance'}
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='qbrain-Hook diagnostic inspection-unit-') as d:
