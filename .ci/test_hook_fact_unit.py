"""Execute real Hook fact context unit tests; no claim of independently measured model egress."""
import argparse
import hashlib
import json
import os
...
from validate_hook_fact_report import validate_unit_payload
...
def main():
    ...
    script=Path(__file__).resolve();test=script.parents[1]/'tests/test_n47d.cpp'
    report={'result':'FAIL','native_windows':os.name=='nt','scope':'synthetic C++ Hook fact context tests; no live host acceptance'}
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='qbrain-Hook fact context-unit-') as d:
