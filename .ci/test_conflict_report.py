"""Negative evidence tests for the N47B process/unit gates."""
import json
from pathlib import Path
import tempfile
import unittest
from unittest.mock import patch
import test_conflict_process as process
from validate_conflict_report import UNIT_SCENARIOS, validate_process, validate_unit, validate_unit_payload
...
class ConflictReportTests(unittest.TestCase):
    ...
    def process_fixture(self):
        r=self.common();r.update(checks=[dict(name=n,status='PASS') for n in sorted(process.EXPECTED_CHECKS)],
            check_count=38,counts={'total':38,'pass':38,'fail':0},
            commands=[dict(exit_code=0,expected_exit=0) for _ in range(75)])
        r['commands'][-1]=dict(exit_code=1,expected_exit=1);return r
    def unit_fixture(self):
        r=self.common();r.update(test_sha256='test',exit_code=0,scenario_count=13,checks=351,
    ...
    def test_complete(self):
        self.assertEqual(self.vp(self.process_fixture()),{'checks':38,'commands':75})
        self.assertEqual(self.vu(self.unit_fixture()),{'scenarios':13,'assertions':351})
    ...
    def test_payload_checked_without_source_attribution(self):
        r=self.unit_fixture();self.assertEqual(validate_unit_payload(r)['scenarios'],13)
        r['scenarios']=[]
    ...
    def test_standalone_copy_does_not_claim_parent_git_source(self):
        with tempfile.TemporaryDirectory() as d:
            script=Path(d)/'verification'/'test_conflict_process.py';script.parent.mkdir();script.write_bytes(b'# fixture')
    ...
    def test_unit_names_match_actual_cpp_registry(self):
        import re
        text=(Path(__file__).resolve().parents[1]/'tests/test_n47b.cpp').read_text(encoding='utf-8')
