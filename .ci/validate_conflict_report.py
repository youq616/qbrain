"""N47B complete process/unit evidence gates. No inference from a PASS label alone."""
from test_conflict_process import EXPECTED_CHECKS, EXPECTED_COMMAND_COUNT

UNIT_SCENARIOS = (
    'lazy conflict view and no automatic contradiction',
    'canonical complete pairs and provenance',
    'source predicate and either-endpoint filters',
    'read-only SQL and no scheduled work',
    'retraction and supersession hide current conflicts',
    'support forgetting and next-call invalidation',
    'invalid or unavailable evidence never yields half a pair',
    'wall-clock expiration hides the complete conflict',
    'two-sided byte and pair limits',
    'candidate limit remains explicit for filtered-out evidence',
    'evidence work cap suppresses incomplete pair',
    'two endpoints share snapshot across concurrent forget',
    'schema errors and foreign key policy fail closed',
)
...
def identity(r,source_commit,binary_sha256,script_sha256,native):
    require(isinstance(r,dict) and r.get('result')=='PASS','conflict test failed')
...
def validate_unit_payload(r):
    ...
    require(type(r.get('checks')) is int and r['checks']>=346 and sum(x['assertions'] for x in rows)==r['checks'],'wrong assertion totals')
