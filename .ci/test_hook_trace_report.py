...
import test_hook_trace_process as suite
from validate_hook_trace_report import validate_process, validate_unit, UNIT_SCENARIOS

class HookTraceReportTests(unittest.TestCase):
    def fixture(self):
        ...
        return dict(result='PASS',source_commit='source',tracked_tree_clean=True,native_windows=True,
            ...
            commands=[dict(name=name,exit_code=0,expected_exit=0) for name in suite.COMMAND_SCHEDULE])
    ...
    def test_duplicate_rejection_scenario_cannot_be_omitted(self):
        r=self.unit()
        r['scenarios']=[s for s in r['scenarios'] if s['name']!='failure phases cannot masquerade as completed processing']
