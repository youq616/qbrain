...
class MultitermReportTests(unittest.TestCase):
    ...
    def process_fixture(self):
        r=self.common();r.update(checks=[dict(name=n,status='PASS') for n in sorted(process.EXPECTED_CHECKS)],
            check_count=61,counts={'total':61,'pass':61,'fail':0},
            commands=[dict(exit_code=0,expected_exit=0) for _ in range(81)])
    ...
    def test_complete(self):
        self.assertEqual(self.vp(self.process_fixture()),{'checks':61,'commands':81})
