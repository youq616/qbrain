...
import test_strict_json_process as suite
from validate_strict_json_report import validate_process, validate_unit, UNIT_SCENARIOS

class StrictJsonReportTests(unittest.TestCase):
    def fixture(self):
        ...
        return dict(result='PASS',source_commit='source',tracked_tree_clean=True,native_windows=True,
            ...
            commands=[dict(args=list(args),exit_code=code,expected_exit=code) for args,code in suite.COMMAND_SCHEDULE])
    ...
    def test_expected_negative_is_required(self):
        r=self.fixture();idx=next(i for i,c in enumerate(r['commands']) if c['expected_exit']==1);self.validate(r)
        r['commands'][idx]['exit_code']=0
        with self.assertRaises(ValueError):self.validate(r)
    def test_expected_failure_cannot_be_relabelled_or_reordered(self):
        r=self.fixture();idx=next(i for i,c in enumerate(r['commands']) if c['expected_exit']==1)
        r['commands'][idx].update(exit_code=0,expected_exit=0)
        with self.assertRaises(ValueError):self.validate(r)
        r=self.fixture();r['commands'][0]['args']=['fact','create']
        with self.assertRaises(ValueError):self.validate(r)
    ...
    def unit(self):
        return dict(result='PASS',source_commit='source',tracked_tree_clean=True,native_windows=True,
            ...
            scenarios=[dict(name=n,status='PASS',assertions=50) for n in UNIT_SCENARIOS],
            scenario_count=len(UNIT_SCENARIOS),checks=50*len(UNIT_SCENARIOS))
    ...
    def test_duplicate_rejection_scenario_cannot_be_omitted(self):
        r=self.unit()
        r['scenarios']=[s for s in r['scenarios'] if s['name']!='duplicates rejected per object including decoded escaped keys']
