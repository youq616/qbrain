"""Negative evidence tests for the N47D process/unit gates."""
import json
from pathlib import Path
import tempfile
import unittest
from unittest.mock import patch
import test_hook_fact_process as process
from validate_hook_fact_report import UNIT_SCENARIOS, validate_process, validate_unit, validate_unit_payload

class HookFactReportTests(unittest.TestCase):
    def common(self):
    ...
    def process_fixture(self):
        r=self.common();r.update(checks=[dict(name=n,status='PASS') for n in sorted(process.EXPECTED_CHECKS)],
            check_count=52,counts={'total':52,'pass':52,'fail':0},
            commands=[dict(exit_code=0,expected_exit=0) for _ in range(72)])
        return r
    ...
    def test_complete(self):
        self.assertEqual(self.vp(self.process_fixture()),{'checks':52,'commands':72})
    ...
    def test_standalone_copy_does_not_claim_parent_git_source(self):
        with tempfile.TemporaryDirectory() as d:
            script=Path(d)/'verification'/'test_hook_fact_process.py';script.parent.mkdir();script.write_bytes(b'# fixture')
    ...
    def test_unit_names_match_actual_cpp_registry(self):
        import re
        text=(Path(__file__).resolve().parents[1]/'tests/test_n47d.cpp').read_text(encoding='utf-8')
    ...
    def vi(self,r):
        from validate_hook_fact_report import validate_install
