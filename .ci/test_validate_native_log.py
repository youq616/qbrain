...
class NativeEvidenceTests(unittest.TestCase):
    ...
    def test_n46b_exact_registry(self):
        ...
        log = ''.join('[PASS] ' + name + '\n' for name in names)
        self.assertEqual(verified_groups(source, log, 55), names)
        with self.assertRaises(ValueError):
            verified_groups(source, log)  # Old default of 44 must not certify 55.
    ...
    def test_n47d_cannot_reuse_51_group_log(self):
        ...
        with self.assertRaises(ValueError):verified_groups(source,log,55)
