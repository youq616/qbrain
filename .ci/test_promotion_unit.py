"""Execute real local fact promotion unit tests; no claim of independently measured model egress."""
import argparse
import hashlib
import json
import os
...
import sys
import tempfile
from test_promotion_process import provenance
from validate_promotion_report import validate_unit_payload
...
def main():
    ...
    script=Path(__file__).resolve();test=script.parents[1]/'tests/test_n47e.cpp'
    report={'result':'FAIL','native_windows':os.name=='nt','scope':'synthetic C++ local fact promotion tests; no live host acceptance'}
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='qbrain-local fact promotion-unit-') as d:
