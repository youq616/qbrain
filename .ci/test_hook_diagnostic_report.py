...
import test_hook_diagnostic_process as suite
from validate_hook_diagnostic_report import validate_process, validate_unit, UNIT_SCENARIOS

class HookDiagnosticReportTests(unittest.TestCase):
    def fixture(self):
        ...
        return dict(result='PASS',source_commit='source',tracked_tree_clean=True,native_windows=True,
            ...
            commands=[dict(name=name,exit_code=2 if name in suite.NEGATIVE else 0,expected_exit=2 if name in suite.NEGATIVE else 0) for name in suite.COMMAND_SCHEDULE])
    ...
    def test_unexpected_or_missing_commands(self):
        for edit in [lambda r:r['commands'].pop(),lambda r:r['commands'][0].update(exit_code=2),lambda r:r['commands'][0].update(exit_code=False),lambda r:r['commands'][0].update(timed_out=True)]:
            r=self.fixture();edit(r)
            with self.assertRaises(ValueError):self.validate(r)
    def test_expected_success_cannot_be_relabelled(self):
        r=self.fixture();r['commands'][0].update(exit_code=1,expected_exit=1)
        with self.assertRaises(ValueError):self.validate(r)
    def test_command_schedule_cannot_be_reordered(self):
        r=self.fixture();r['commands'].reverse()
        with self.assertRaises(ValueError):self.validate(r)
        r=self.fixture();r['commands'][0]['name']='invented'
        with self.assertRaises(ValueError):self.validate(r)
    ...
    def unit(self):
        return dict(result='PASS',source_commit='source',tracked_tree_clean=True,native_windows=True,
            ...
            scenarios=[dict(name=n,status='PASS',assertions=7 if i==0 else 10) for i,n in enumerate(UNIT_SCENARIOS)],
            scenario_count=len(UNIT_SCENARIOS),checks=107)
    ...
    def test_duplicate_rejection_scenario_cannot_be_omitted(self):
        r=self.unit()
        r['scenarios']=[s for s in r['scenarios'] if s['name']!='raw duplicate keys malformed UTF8 and depth are rejected']
    ...
    def test_forgotten_case_cannot_be_dropped_with_adjusted_totals(self):
        r=self.unit();r['scenarios']=[s for s in r['scenarios'] if s['name']!='forgotten replay and failed phases never become consumption']
    ...
    def test_config_rejection_cannot_be_relabelled_success(self):
        r=self.fixture()
        c=next(c for c in r['commands'] if c['name'] in suite.NEGATIVE)
        c.update(exit_code=0,expected_exit=0)
        with self.assertRaises(ValueError):self.validate(r)
